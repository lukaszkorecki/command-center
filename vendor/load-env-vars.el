...
;; Vendored from https://github.com/lukaszkorecki/emacs-load-env-vars
;; (rev ccdd7cd9a0abb79c9d3b327b792e192bb87e2eba) to avoid MELPA's
;; older 0.0.2 release overwriting the fork's `load-env-vars-from-string'.
...
(defun load-env-vars-set-env (env-vars)
  ...
  (dolist (element env-vars)
    (let ((key (car element)) (value (cadr element)))
      (message "load-env-vars - setting %s" key)
      (setenv key value))))
